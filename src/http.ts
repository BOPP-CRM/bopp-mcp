...
function getApiKeyFromAuth(req: Request): string | undefined {
...
async function handleMcpRequest(
  req: Request,
  res: Response,
  handler: (apiKey: string) => Promise<void>,
): Promise<void> {
  const apiKey = getApiKeyFromAuth(req);
  if (!apiKey) {
    console.warn(
      "[mcp] 401 missing apiKey clientId=%s",
      req.auth?.clientId ?? "unknown",
    );
    res.status(401).json({
      jsonrpc: "2.0",
